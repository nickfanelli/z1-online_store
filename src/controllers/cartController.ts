...
export class CartController {
  static async viewCart(req: Request, res: Response): Promise<void> {
    ...
    const cartJSONResponse = cart.toJSON();
    
    // Adding 'cartTotal' to the final JSON response, using the the spread operator('...')
    const extendedCartJSONResponse = { ...cartJSONResponse, ...{ cartTotal: total }}

    res.json(extendedCartJSONResponse);
  }
